...
def index_paginate(page_num, per_page):
    images = Image.query.order_by(db.desc(Image.id)).paginate(page=page_num, per_page=per_page, error_out=False)
    ...
    for item in images.items:
        ...
        imgov = {'image_user_id': item.user.id,
...
def upload():
    ...
    if file_ext in app.config['ALLOWED_EXT']:
        ...
        #七牛云上的图片访问连接
        url = 'http://ovlu9q6wc.bkt.clouddn.com/'+qiniu_upload_file(file, file_name)
